...
# ====   simulateTradingResult function ==

## buy sell at the next_close version, use 4MA and KD (4MA begin up and K>D && 4MA begin down and K<D)
simulateTradingResult_4MA_KD <- function( beginDay_index, endDay_index, upLimit, downLimit){
    ...
    for(index in monthBC:monthEC){
      ...
      if (nrow(testDay_pred) != 0){  # this month doesn't have this day
        ...
        for (shiftT in 2:(nrow(testDay_pred)-1) ){
          ...
          #
          if ( testDay_pred[shiftT,]$EMA4_flag == 1 && testDay_pred[shiftT-1,]$EMA4_flag == -1 && (testDay_pred[shiftT,]$frontDay_dayKValue > testDay_pred[shiftT,]$frontDay_dayDValue) ){# && (testDay_pred[shiftT-1,]$KValue <= 50) && (testDay_pred[shiftT-1,]$DValue <= 50) ){
            updownFlag <- 1
...
fitnessFunction <- function(x){
  ...
  returnList <- simulateTradingResult_4MA_KD( beginMIndex, endMIndex, upLimit, downLimit)
  ...
  cat( (totalUpProfit - totalDownProfit) / (totalUpNum + totalDownNum), " ,totalUpProfit = ", totalUpProfit, ", totalDownProfit = ", totalDownProfit, ", totalUpNum = ", totalUpNum, ", totalDownNum", totalDownNum, "\n")
...
for (a in 11:(tradingYMCount-2)){  # (tradingYMCount-2)
  beginMIndex <- a
  endMIndex <- a + 1
  test_index <- a + 2 
  
  GAmodel <- rbga.bin(size = upSize + downSize , popSize = 40, iters = iter, mutationChance = 0.01, 
                      elitism = T, evalFunc = fitnessFunction)
  ...
  returnList_test <- simulateTradingResult_4MA_KD( test_index, test_index, upLimit_test, downLimit_test)
...
save.image("strategyTrading_4EMA_KD_result.Rdata")